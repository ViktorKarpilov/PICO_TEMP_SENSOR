...
int DhcpServer::init(const ip4_addr_t& server_ip, const ip4_addr_t& netmask, std::uint8_t base_ip) {
    ...
    udp_recv(control_block, udp_process_request_function, this);
...
udp_recv_fn DhcpServer::udp_process_request_function = [](void* arg, struct udp_pcb* pcb, 
                                                         struct pbuf* packet, 
...
bool DhcpServer::is_lease_expired(const dhcp_lease_t& lease) {
    const std::uint32_t current_time = cyw43_hal_ticks_ms() >> 16;
...
void DhcpServer::process_dhcp_request(const dhcp_message_t& request, 
    ...
    leases[lease_index].expiry = (cyw43_hal_ticks_ms() + CONFIG::DEFAULT_LEASE_TIME_S * 1000) >> 16;
...
pbuf* DhcpServer::create_dhcp_response(const dhcp_message_t& request, 
    ...
    *resp_msg = request;
    ...
    resp_msg->yiaddr[0] = ip4_addr1(&server_ip);
    resp_msg->yiaddr[1] = ip4_addr2(&server_ip);
    resp_msg->yiaddr[2] = ip4_addr3(&server_ip);
